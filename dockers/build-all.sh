...
then
  ...
  docker run -dt -e toolchain=${toolchain} --name gateway_dependencies -v "${PWD}:/src:ro,Z" madmongo1/fedora-devel:latest
  docker exec -dt gateway_dependencies /bin/bash /src/dockers/build-deps.sh
fi
