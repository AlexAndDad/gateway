...
namespace
{
    minecraft::chunks::chunk_column make_test_chunk()
    {
        auto col        = minecraft::chunks::chunk_column();
        auto fill_layer = [&col](int y, auto blk_id) {
            for (int x = 0; x < minecraft::chunks::chunk_column::x_extent; ++x)
                for (int z = 0; z < minecraft::chunks::chunk_column::z_extent;
                     ++z)
                    col.change_block(minecraft::chunks::vector3(x, y, z),
                                     blk_id.to_id(),
                                     false);
        };
          ...
          "[minecraft::chunks::provider][minecraft::chunks][minecraft]")
{
    ...
    SECTION("Cancel consumer")
    {
        ...
        SECTION("Next wait should not fail")
        {
            ...
            CHECK(holds_alternative< std::unique_ptr< chunks::chunk_column > >(
                update.as_variant()));
        }
    }
    ...
    SECTION("Snapshot ordering")
    {
        ...
        auto provider = chunks::provider(ioc.get_executor());
        auto consumer =
            provider.connect(net::make_strand(net::system_executor())); // 1 post
        ...
        provider.notify(chunks::vector3(0, 128, 0),
                        blocks::birch_wood().to_id()); // 1 post
        CHECK(run(ioc) == 2);
        ...
        CHECK(u.new_state == blocks::birch_wood().to_id());

        for (int x = 0; x < 16; ++x)
            for (int y = 0; y < 16; ++y)
                provider.notify(chunks::vector3(x, 128, y),
                                blocks::birch_wood().to_id());
