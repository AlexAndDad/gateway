...
#include "minecraft/server/player_position_and_look.hpp"
...
namespace minecraft::server
{
    ...
    void compose(play_packet const &p, std::vector< char > &buf)
    {
    ...
    void compose(unique_play_packet const &p, std::vector< char > &buf)
    {
        assert(p);
        compose(*p, buf);
    }
    ...
    void compose(shared_play_packet const &p, std::vector< char > &buf)
    {
        assert(p);
        compose(*p, buf);
    }
